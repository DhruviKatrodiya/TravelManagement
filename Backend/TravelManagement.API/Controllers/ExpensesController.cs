...
public class ExpensesController : ControllerBase
{
    ...
    [HttpGet] public async Task<ActionResult<ApiResponse<IEnumerable<ExpenseDto>>>> List([FromQuery] int? bookingId, [FromQuery] DateTime? from, [FromQuery] DateTime? to)
        => Ok(ApiResponse<IEnumerable<ExpenseDto>>.Ok(await _svc.ListAsync(bookingId, from, to)));

    [HttpPost] public async Task<ActionResult<ApiResponse<ExpenseDto>>> Create(ExpenseCreateRequest req)
        => Ok(ApiResponse<ExpenseDto>.Ok(await _svc.CreateAsync(req), "Expense recorded"));

    [HttpPut("{id}")] public async Task<ActionResult<ApiResponse<ExpenseDto>>> Update(int id, ExpenseCreateRequest req)
    {
