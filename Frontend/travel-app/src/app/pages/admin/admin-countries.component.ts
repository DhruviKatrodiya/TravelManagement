...
@Component({
  ...
  template: `
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <button *ngIf="auth.isAdmin()" class="btn btn-primary" (click)="startCreate()"><i class="bi bi-plus-lg me-1"></i>Add country</button>
    </div>
    ...
    <div *ngIf="editingId !== null" class="modal fade show d-block" tabindex="-1" role="dialog">
      <div class="modal-dialog modal-dialog-centered" (click)="$event.stopPropagation()">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title fw-bold">{{ editingId ? 'Edit country' : 'New country' }}</h5>
          </div>
            ...
            <div class="modal-body" #modalBody>
              ...
              <div class="row g-3">
                <div class="col-md-8">
                  ...
                  <input class="form-control" formControlName="name" placeholder="e.g. India" [class.is-invalid]="isInvalid(form.get('name'))" />
                  <div class="invalid-feedback" *ngIf="isInvalid(form.get('name'))">Country name is required.</div>
                </div>
                <div class="col-md-4">
                  <label class="form-label">Code</label>
                  <input class="form-control" formControlName="code" placeholder="e.g. IN" maxlength="10" />
                  <small class="text-muted">ISO country code</small>
                </div>
              </div>
            </div>
            <div class="modal-footer">
    ...
    <div class="table-card mb-3">
      <div class="row g-2 align-items-end">
        <div class="col-md-6">
          <label class="form-label small text-muted mb-1">Search country</label>
        ...
        <table class="table align-middle">
          ...
          <tbody>
            <tr *ngFor="let c of pagedItems()">
              ...
              <td class="text-end">
                <div class="d-flex gap-1 justify-content-end">
                  <button *ngIf="auth.isAdmin()" class="btn btn-sm btn-outline-secondary" (click)="edit(c)"><i class="bi bi-pencil me-1"></i>Edit</button>
                  <button *ngIf="c.isActive && auth.isAdmin()" class="btn btn-sm btn-outline-danger" (click)="remove(c)" [disabled]="togglingId === c.id">
                    <i class="bi bi-eye-slash me-1"></i>Deactivate
                  </button>
                  <button *ngIf="!c.isActive && auth.isAdmin()" class="btn btn-sm btn-outline-success" (click)="activate(c)" [disabled]="togglingId === c.id">
            ...
            <tr *ngIf="filteredItems().length === 0">
              <td colspan="4" class="text-center text-muted py-3">{{ items.length === 0 ? 'No countries yet.' : 'No countries match the filters.' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export class AdminCountriesComponent implements OnInit, OnDestroy {
  ...
  load(): void {
    this.api.listCountries().subscribe({ next: cs => { this.items = cs; this.clampPage(); } });
  }
  ...
  pagedItems(): Country[] { const s = (this.page - 1) * this.pageSize; return this.filteredItems().slice(s, s + this.pageSize); }
  ...
  edit(c: Country): void {
    this.editingId = c.id;
    this.formError = '';
    this.form.reset({ name: c.name, code: c.code || '' });
    this.lockBody();
  }
  ...
  save(): void {
    ...
    const op = this.editingId ? this.api.updateCountry(this.editingId, v) : this.api.createCountry(v);
    op.subscribe({
      next: () => {
        this.toast.show(this.editingId ? 'Country updated.' : 'Country added.', 'success', 3000);
        this.cancel();
        this.load();
      },
