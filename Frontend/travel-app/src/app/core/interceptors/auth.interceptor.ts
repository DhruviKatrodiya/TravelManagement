...
export const authInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  // /auth/me is intentionally NOT excluded so a 401 from session-check triggers auto-logout
  const isAuthEndpoint = /\/auth\/(login|register|forgot-password|verify-forgot-password|send-change-password-otp|change-password)$/.test(req.url);
  ...
  return next(cloned).pipe(
    catchError((err: HttpErrorResponse) => {
      ...
      if (err.status === 401) {
        ...
        toast.show('Cannot reach API. Make sure backend is running on https://localhost:7138/api.', 'danger', 6000, { title: 'Connection error', persist: false });
      } else {
